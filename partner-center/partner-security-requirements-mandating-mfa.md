...
**Juiste rollen:** beheeragent | Verkoopagent | Helpdeskagent | Factureringsbeheerder | Globale beheerder
...
| Alle pagina's op > tabblad Klantaanvragen     | x      |       |    x   |       |       |
| Pagina Facturering     |   x    |       |       |    x   |   x    |

Als u toegang probeert te krijgen tot een van deze pagina's en u de MFA-verificatie nog niet eerder hebt voltooid, moet u dit doen. Andere pagina's Partner Center zoals de pagina Overzicht, Service Health pagina Statuscontrole vereisen geen MFA.
...
3. Vanwege de bestaande Azure AD MFA-installatie door Contoso is Jane vereist om MFA-verificatie te voltooien. Na geslaagde aanmelding en MFA-verificatie wordt Jane teruggeleid naar Partner Center dashboardoverzichtspagina.

4. Jane probeert toegang te krijgen tot een van de met MFA beveiligde pagina's in Partner Center. Omdat Jane de MFA-verificatie al eerder heeft voltooid tijdens het aanmelden, heeft Jane toegang tot de met MFA beveiligde pagina zonder dat ze MFA-verificatie opnieuw moet door voeren.

### <a name="example-2-partner-has-implemented-third-party-mfa-using-identity-federation"></a>Voorbeeld 2: Partner heeft MFA van derden geïmplementeerd met identiteitsfederatie
...
4. Er wordt geprobeerd toegang te krijgen tot een van de met MFA beveiligde pagina's in Partner Center. Omdat Dev al eerder MFA-verificatie heeft voltooid tijdens het aanmelden, heeft Hij toegang tot de met MFA beveiligde pagina zonder dat MFA-verificatie opnieuw moet worden uitgevoerd.
...
6. John probeert toegang te krijgen tot een van de met MFA beveiligde pagina's in Partner Center. Omdat John de MFA-verificatie niet heeft voltooid, Partner Center John omgeleid naar Azure AD om de MFA-verificatie te voltooien. Omdat John MFA heeft geregistreerd, wordt hij deze keer alleen gevraagd om de MFA-verificatie te voltooien.
...
Wanneer App+ Gebruikersverificatie wordt gebruikt, Partner Center MFA-verificatie vereist. Om precies te zijn: wanneer een partnertoepassing een API-aanvraag naar Partner Center wil verzenden, moet deze een toegang token bevatten in de autorisatie-header van de aanvraag. 
...
Partneraccounts, waaronder beheerdersagents en helpdeskmedewerkers, kunnen de gedelegeerde beheerdersbevoegdheden van hun partner gebruiken om klantresources te beheren via Microsoft Online Services-portals, opdrachtregelinterface (CLI) en API's (met behulp van App+Gebruikersverificatie).
...
- Partner moet voorkomen dat niet-interactieve gebruikersverificatiemethode met Azure AD wordt gebruikt om het toegangsken te verkrijgen. Wanneer u niet-interactieve gebruikersverificatiemethode zoals [Password Flow](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/wiki/Acquiring-tokens-with-username-and-password)gebruikt, kan azure AD de gebruiker niet vragen om de MFA-verificatie te voltooien. De partner moet overschakelen naar een interactieve gebruikersverificatiemethode, zoals [OpenID Connect stroom.](/azure/active-directory/develop/v1-protocols-openid-connect-code)

- Tijdens de interactieve verificatiemethode voor gebruikers moet de partner een partnergebruikersaccount gebruiken dat al is ingeschakeld voor MFA. Als u hier om wordt gevraagd door Azure AD, kan de partner ook MFA-registratie en MFA-verificatie voltooien tijdens het aanmelden.

- Dit is vergelijkbaar met het scenario waarin een eindklantten tenant MFA heeft geïmplementeerd voor de beheerders. De tenant van de klant heeft bijvoorbeeld [standaardinstellingen](/azure/active-directory/fundamentals/concept-fundamentals-security-defaults)voor Azure AD-beveiliging ingeschakeld. Hiervoor zijn alle gebruikersaccounts met beheerdersrechten vereist om zich aan te melden bij de tenant van de klant met MFA-verificatie, inclusief beheerdersagents en helpdeskmedewerkers. Voor testdoeleinden kunnen partners de [standaardinstellingen](/azure/active-directory/fundamentals/concept-fundamentals-security-defaults) voor Azure AD-beveiliging inschakelen in de tenant van de klant en vervolgens proberen de gedelegeerde beheerdersbevoegdheden van de partner te gebruiken om programmatisch toegang te krijgen tot de tenant van de klant.

### <a name="mfa-registration-experience"></a>MFA-registratie-ervaring

Als het partneraccount tijdens MFA-verificatie nog niet eerder is geregistreerd voor MFA, wordt de gebruiker eerst door Azure AD gevraagd om de MFA-registratie te voltooien:
...
Een partner heeft een aantal partneragents die nog gebruikmaken van verouderde verificatieprotocollen, die niet compatibel zijn met MFA. De gebruikers maken bijvoorbeeld nog steeds gebruik van Outlook 2010, dat is gebaseerd op verouderde verificatieprotocollen. Het inschakelen van MFA voor deze partneragents verstoort het gebruik van verouderde verificatieprotocollen.

**Antwoord:** Nee, dit is geen geldige reden voor een technische uitzondering. Partners worden ten zeerste aangeraden af te zien van het gebruik van verouderde verificatieprotocollen vanwege mogelijke gevolgen voor de beveiliging, omdat deze protocollen niet kunnen worden beveiligd met MFA-verificatie en veel vatbaarder zijn voor referentierisico's. Als het niet mogelijk is om af te zien van het gebruik van verouderde verificatieprotocollen, moeten partners overwegen om zich aan te melden voor Azure AD Premium, dat ondersteuning biedt voor het gebruik van toepassingswachtwoorden. Toepassingswachtwoorden zijn een een keer door het systeem gegenereerde wachtwoorden en zijn meestal sterker dan door mensen gegenereerde wachtwoorden. Met behulp van toepassingswachtwoorden kunnen partners MFA implementeren voor hun gebruikers, terwijl ze alleen voor verouderde verificatieprotocollen terugvallen op toepassingswachtwoorden.
...
Een aanvraag voor technische uitzondering indienen:

1. Meld u aan Partner Center globale beheerder of beheerderagent.

2. Maak een nieuwe partnerserviceaanvraag door te navigeren naar Ondersteuningsaanvragen  >  **van partners** en Nieuwe aanvraag te **selecteren.**

3. Zoeken naar **MFA- Uitzonderingsaanvraag** in het zoekvak; of selecteer **CSP** in Categorie, selecteer **vervolgens Accounts, Onboarding, Toegang** vanuit onderwerp, selecteer **vervolgens MFA - Aanvraag** voor uitzondering van het subonderwerp en selecteer vervolgens volgende **stap.**
...
Het kan tot drie werkdagen duren voordat Microsoft een reactie geeft op een aanvraag voor technische uitzondering.
