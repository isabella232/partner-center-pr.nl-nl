...
2. Er wordt een nieuwe browsersessie gestart en er wordt Partner Center naar de overzichtspagina van het dashboard (die niet met MFA is beveiligd). Partner Center omgeleid naar Azure AD om u aan te melden.
...
3. Omdat Fabrikam MFA niet heeft geïmplementeerd, hoeft John de MFA-verificatie niet te voltooien. Na een geslaagde aanmelding wordt John teruggeleid naar Partner Center dashboardoverzichtspagina.
...
##### <a name="issue-2-partner-has-not-implemented-mfa-for-user-accounts-not-using-delegated-admin-privileges"></a>Probleem 2: De partner heeft MFA niet geïmplementeerd voor gebruikersaccounts die geen gedelegeerde beheerdersbevoegdheden gebruiken
