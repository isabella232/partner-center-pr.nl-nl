...
3. Omdat Wingtip identiteitsfederatie heeft ingesteld, leidt Azure ADPrint om naar de federatief-id-provider om aanmelding en MFA-verificatie te voltooien. Na een geslaagde aanmelding en MFA-verificatie wordt Hij teruggeleid naar Azure AD en vervolgens naar Partner Center dashboardoverzichtspagina.
...
Wanneer Partner Center een API-aanvraag ontvangt met een toegangs token dat is verkregen met behulp van App+User-verificatie, controleert de Partner Center-API of de *MFA-waarde* aanwezig is in de *CLAIM Authentication Method Reference (AMR).* U kunt een JWT-decoder gebruiken om te controleren of een toegangsteken de verwachte amr-waarde (Authentication Method Reference) bevat of niet:
...
- De partner kan zich ook aanmelden voor Azure AD Premium MFA-oplossingen van derden (compatibel met Azure AD) die aanvullende verificatiemethoden kunnen bieden.
