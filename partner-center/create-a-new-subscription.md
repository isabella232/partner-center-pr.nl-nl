...
Sommige aanbiedingen zijn beperkt tot één abonnement per klant. Als u een lijst wilt bekijken met aanbiedingen die beperkt zijn, gaat u naar Partner Center pagina Prijzen en aanbiedingen.
...
4. Als u alleen bepaalde typen abonnementen wilt zien, maakt u selecties in de beschikbare filters:
   ...
   - **Factureringstype:** selecteer het type abonnement dat u wilt gebruiken: **Licentie** of **Gebruik.** Zie [Facturering op basis van licenties voor](license-based-billing.md) informatie over het kiezen tussen de maandelijkse en jaarlijkse factureringsfrequentie.
...
Voor nieuwe commerceaanbiedingen kunt u het abonnement op elk moment vóór de periode voor de termijn annuleren. Wanneer u een abonnement annuleert, heeft de klant onmiddellijk geen toegang meer tot de service. De toegang kan niet worden hersteld na annulering. De volgende annuleringsopties zijn beschikbaar voor een partner nadat het abonnement is aangeschaft: 
...
3. Lees de modale pop-up en selecteer **OK**
...
>[!IMPORTANT] 
> Als u een abonnement onderpauzeert, worden alle instellingen voor automatisch verlengen uitgeschakeld en worden eventuele bestaande wijzigingen verwijderd. Het gebruik van een subscriptoin heeft alleen invloed op de toegang tot de service van de klant. De facturering van de partner blijft in de onderbroken status.
...
Een nieuwe wijziging plannen die moet worden uitgevoerd tijdens de verlenging
...
> [!Note] 
>- Proefversies worden standaard ge converteerd naar de betaalde SKU aan het einde van hun termijn. 
...
Met gedeeltelijke upgrades kan een partner bepaalde licenties van de ene SKU naar de andere aanwijzen. Met de vorige upgrade in traditionele abonnementen op basis van licenties konden alleen alle licenties worden bijgewerkt. Nieuwe handel stelt een partner in staat om bepaalde licenties op hun gemak te verplaatsen. Dit geeft de partner meer controle over het beheren van upgrades, zodat ze sommige gebruikers naar een nieuwe SKU kunnen verplaatsen zonder ze allemaal te verplaatsen. 

Gedeeltelijke upgrades kunnen aan het einde van een termijn worden gepland of kunnen halverwege de termijn worden gestart. 

Details van gedeeltelijke upgrade:

- Deze optie is gedeeltelijk gedefinieerd als het aantal upgradelicenties anders is dan het oorspronkelijke abonnement.
- Bij het initiëren van upgrades op de korte termijn worden bestaande geplande upgrades verwijderd.
- Upgrades kunnen alleen worden gestart vanuit abonnementen met de **status** Actief.
- Een nieuw abonnement dat is gemaakt tijdens de upgrade, heeft dezelfde einddatum als het abonnement waar de upgrade van afkomstig is.

Partners hebben toegang tot het abonnement waar ze een upgrade naar willen uitvoeren bij het configureren van het aantal licenties en het abonnement waar ze een upgrade naar willen uitvoeren. Partners kunnen een nieuw **abonnement** selecteren of een bestaand abonnement selecteren.
