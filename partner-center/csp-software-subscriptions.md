...
3. Voer het datumbereik in voor de activiteit die u wilt zien. In het activiteitenlogboek wordt een lijst weergegeven met gebruikers die tijdens de opgegeven periode softwaresleutels of gedownloade software hebben gebruikt.

## <a name="toggle-auto-renew"></a>Automatisch verlengen in-/uitschakelen

Vanaf 14 oktober 2021 worden nieuw gekochte softwareabonnementen standaard ingesteld op automatische verlenging op de aangewezen verlengingsdatum. Dit onderdeel voor automatisch verlengen kan worden in- en uitgeschakeld om te voldoen aan de behoeften van de partner. Voor softwareabonnementen die zijn aangeschaft vóór 14 oktober 2021, is de standaardinstelling voor automatisch verlengen niet ingeschakeld en komt het niet in aanmerking voor automatische verlenging om te worden toegepast.

## <a name="managing-billing-frequency"></a>Factureringsfrequentie beheren

Wijzigingen in de factureringsfrequentie halverwege de termijn worden ondersteund voor alle aankopen van softwareabonnementen vanaf 14 oktober 2021. Een wijziging van de maandelijkse naar jaarlijkse factureringsfrequentie wordt ondersteund voor alle softwareabonnementen. Wijzigingen in de facturering per jaar en per maand worden alleen ondersteund als de resterende duur van het softwareabonnement langer is dan één jaar. Tijdens het eerste jaar van de termijn kan bijvoorbeeld een softwareabonnement voor een periode van drie jaar met jaarlijkse facturering worden gewijzigd in maandelijkse facturering nadat de eerste jaarlijkse betaling is verricht; na het eerste jaar dat is betaald, geven de volgende factureringskosten een maandelijkse factureringscyclus en -prijs weer. 

Alle factureringswijzigingen worden van kracht nadat de huidige factureringscyclus is voltooid en een softwareabonnement is aan. 
...
Wanneer u een order probeert te annuleren, Partner Center u mogelijk ook andere informatie (die wordt weergegeven onder de vervolgkeuzelijst Ordernummer). Deze informatie kan omvatten:
...
- Als we meer informatie over uw annuleringsaanvraag nodig hebben, krijgt u mogelijk een koppeling naar een **klantondersteuningsaanvraagformulier.**

> [!IMPORTANT]
> Nadat u een order hebt geannuleerd, wordt er een bericht weergegeven waarin de annulering wordt bevestigd. Er kan echter een vertraging van maximaal 15 minuten zijn voordat de annulering wordt weergegeven op het Partner Center dashboard.
...
1. Wanneer u de koppeling **klantondersteuningsaanvraag** selecteert in het venster Aankoop annuleren, wordt de **pagina** Partner Center probleem melden geopend.
...
- [Handleiding voor bestellen en afhandeling via Partner Center](https://partner.microsoft.com/resources/detail/guide-to-ordering-and-fulfillment-through-partner-center-pdf)
