...
- Gebruik de algemene zoekopdracht balk in het bovenste navigatiegebied om een specifieke SaaS-aanbieding te vinden.

Zoek een [privé-SaaS-aanbieding door](./private-plans.md) de banner boven aan de **marketplace-startpagina te** selecteren. Niet alle aanbiedingen of abonnementen zijn beschikbaar in alle geografische gebieden en sommige worden mogelijk alleen weergegeven voor bepaalde tenants.

In de gefilterde weergave wordt elke beschikbare SaaS-aanbieding weergegeven die wordt vertegenwoordigd door een titel. Selecteer er een om de pagina met productdetails weer te geven. Deze bevat de volgende secties:
...
2. Als onderdeel van het aankoopproces wordt u  omgeleid naar het tabblad Basisinformatie en moet u het volgende doen:
    ...
    1. Typ een *Naam voor* SaaS-abonnement om dit later gemakkelijk te kunnen identificeren. Na aanschaf kunt u de naam niet meer wijzigen.
...
4. Ga door **naar Beoordelen en abonneren om** de aanbieding en plandetails te bekijken.
...
- Het *e-mailadres van de* client met object-id *ObjectID* is niet autorisatie om de actie *DeploymentValidationAction* uit te voeren via scope *ResourceGroup; DeploymentScope* of het bereik is ongeldig.  
  - U krijgt dit bericht als u niet de juiste machtigingen hebt voor het Azure-abonnement/de resourcegroep.  
    ...
    Als u resources wilt implementeren in een resourcegroep, moet u ten minste inzenderstoegang hebben. Controleer uw toegangsstatus onder **Resourcegroepen** **en** Access Control. Hier ziet u wie de 'eigenaar' is, die u kunt vragen om u toe te wijzen als 'Inzender'.
